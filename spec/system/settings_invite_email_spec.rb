...
RSpec.describe 'Settings - Client invite email', type: :system, js: true do
  ...
  def expand_invite_section
    find('[role="button"]', text: 'Client invite email').click
  end

  describe 'invite email settings card' do
    it 'is visible on the settings page with summary info' do
      user
      visit settings_path

      # Heading and summary always visible in collapsed state
      expect(page).to have_content('Client invite email and template')
      expect(page).to have_content('Auto-send: Disabled')
      expect(page).to have_button('Manage')
    end

    it 'shows subject and body template inputs with variable buttons when expanded' do
      user
      visit settings_path
      expand_invite_section
    ...
    it 'shows the live preview section when expanded' do
      user
      visit settings_path
      expand_invite_section

      # Text is styled with CSS `uppercase` so Capybara sees it in uppercase
      expect(page).to have_content(/preview \(sample data\)/i)
      expect(page).to have_content('Subject:')
    end

    it 'shows the Save button when expanded' do
      user
      visit settings_path
      expand_invite_section
  ...
  describe 'toggling auto-send' do
    it 'starts with auto-send off and can be enabled' do
      ...
      toggle = find('button[aria-label="Toggle automatic client invite email"]')
      expect(toggle['aria-checked']).to eq('false')

      toggle.click

      expect(find('button[aria-label="Toggle automatic client invite email"]')['aria-checked']).to eq('true')
    end
  end
  ...
  describe 'saving settings' do
    it 'persists a custom subject template' do
      ...
      # After Inertia navigation the section collapses; re-expand to verify saved value
      expand_invite_section
  ...
  describe 'with a pre-existing setting' do
    ...
    it 'pre-fills the form with saved values' do
      ...
      expect(page).to have_selector(
        'button[aria-label="Toggle automatic client invite email"][aria-checked="true"]'
      )
    end
