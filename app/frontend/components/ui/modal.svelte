  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" role="dialog" aria-modal="true" aria-label={title ? `${title} dialog` : 'Dialog'}>
    <div class="bg-white p-6 rounded-md w-full max-w-md">
      ...
      <div class="mt-4 flex justify-end gap-2">
        ...
        <button class="px-3 py-1 rounded bg-red-600 text-white disabled:opacity-50 disabled:cursor-not-allowed" onclick={confirm} type="button" aria-label="Confirm" disabled={confirmDisabled}>{confirmText}</button>
      </div>
    </div>
  </div>
{/if}
